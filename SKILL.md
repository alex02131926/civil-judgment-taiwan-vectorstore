...
- **DO NOT** modify the heading patterns in `ingest.py` or SKILL.md. They are intentionally conservative.
...
- 台灣判決書 section headings 格式不統一（e.g.「理　由」with fullwidth space）。目前採 exact line match；未來可放寬為 regex。
